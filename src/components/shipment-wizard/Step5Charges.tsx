  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <p className="text-sm text-muted-foreground mb-6">Define charges and billing details</p>
        ...
        <div className="flex items-center justify-between mt-4">
          <Button type="button" variant="outline" size="sm" onClick={() => append({ chargeCode: '', description: '', rate: 0, unit: 'Shipment', quantity: 1, amount: 0, ppcc: 'PP' })}>
            <Plus className="mr-1.5 h-4 w-4" /> Add Charge
          </Button>
          <div className="flex items-center gap-3">
            {totalCharges < 0 && (
              <span className="flex items-center gap-1 text-xs text-destructive">
                <AlertTriangle className="h-3.5 w-3.5" /> Negative margin
              </span>
            )}
            <div className="text-right">
              ...
              <p className="text-lg font-bold text-foreground">
                ...
                {formatCurrency(totalCharges, currency).replace(/[^0-9.,]/g, '')}
              </p>
            </div>
          </div>
        </div>
